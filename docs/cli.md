...
| `observe <container> (--prompt <TEXT> \| --tokens <IDS>) --record <FILE> [--heads [--heads-top-k <N>]]` | A prompt in, a run record out (V3-OBS-1 + V3-STREAM-1). Tokenises with the container's own tokenizer (or takes ids), prepares the image once, steps every token through the canonical decode session with the lossless recorder and the stats observer attached — carrier and delta norms, a fixed projection (`--basis-dims`/`--basis-seed`, or `--basis-rows <JSON>` for a registered reader with `--basis-id`), the run's provenance and a verified receipt — and writes it as JSON lines. `--generate <N>` continues greedily with those positions observed too. Prints the ids that ran, the provenance fingerprint, the log hash and the final position's top `--top-k` candidates with log-probabilities. `--backend` as for `exec` (default `production`). `--lens-tokens <IDS>` arms the true logit lens (V3-LENS-1): at every armed site (`--lens-layers all\|every:<k>\|<list>`, FFN sites, plus `--lens-attention`) the image's own final norm and head read the layer output and each token's log-probability and rank go on the record, one full head pass per armed site per position — priced on the receipt and in the printed per-depth table. Never a second traversal. |
